"""
실습 풀이: OPEN API로 받은 주간 날씨(JSON) 파싱하기

핵심: API의 'daily'는 평행한 list 묶음 → zip()으로 하루치 dict들의 list로 재구성.
"""
...
# 1) daily(dict) 안에서 평행한 세 list 꺼내기
daily = data["daily"]
dates = daily["time"]
highs = daily["temperature_2m_max"]
lows = daily["temperature_2m_min"]

# 2) zip으로 같은 인덱스끼리 묶어 → 하루치 dict들의 list 만들기
days = []
...
#   각 dict에서 strEvent, intHomeScore, intAwayScore, dateEvent 를 꺼내 출력!

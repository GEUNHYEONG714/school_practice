...
#       보통 PC에서는 실행하기 어렵다. 1천만 개만으로도 차이를 충분히 볼 수 있다.
N = 10_000_000          # 데이터 개수: 1천만
SEARCH_COUNT = 100      # 검색을 반복해 시간 차이가 잘 보이게 한다.
...
# 일부러 "없는 값"(-1)을 찾는다.
# list는 끝까지 확인해야 하므로 가장 오래 걸리는 경우가 된다.
target = -1
...
# 핵심: 데이터가 늘어날수록 list는 확인할 항목도 함께 늘어난다.
# 반면 set은 hash를 이용하므로 검색 시간이 크게 늘지 않는다.
print("\n" + "=" * 58)

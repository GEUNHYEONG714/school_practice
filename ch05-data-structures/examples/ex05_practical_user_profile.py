...
#
# 회원 한 명에게 태그, 주문 내역처럼 여러 항목이 연결될 때는
# dict의 값으로 list를 넣어 표현할 수 있다.

# 회원 한 명의 정보를 하나의 dict로 묶는다.
member = {
    "id": 1001,
    "name": "김민수",
    "tags": ["신규회원", "이메일인증"],       # 태그가 여러 개라서 list
...
print()
# --- 2. list에 항목 추가/제거 ---
member["tags"].append("VIP")               # 등급이 올라가서 태그 추가
member["tags"].remove("신규회원")            # 더 이상 신규 회원이 아니므로 제거
print("변경된 태그:", member["tags"])


print()
# --- 3. dict가 담긴 list 순회 (주문 내역 출력) ---
print("[주문 내역]")
total = 0
for order in member["orders"]:             # 주문 하나하나가 dict

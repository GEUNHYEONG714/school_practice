...
# 한 키에 여러 값을 담고, 나중에 수정도 해야 할 때 list를 사용한다.
student = {
    "name": "김민수",
    "hobbies": ["축구", "독서", "게임"],   # 값이 리스트
}

print(student["hobbies"])          # ['축구', '독서', '게임']
print(student["hobbies"][0])       # 첫 번째 취미 → 축구

student["hobbies"].append("요리")   # 리스트라서 항목 추가 가능
print(student["hobbies"])          # ['축구', '독서', '게임', '요리']


print()
# --- 2. 값이 튜플(tuple)인 dict ---
# 좌표나 날짜처럼 의미가 정해진 묶음에는 tuple을 자주 사용한다.
city = {
    "name": "서울",
    "location": (37.5665, 126.9780),   # (위도, 경도) — 바뀌면 안 됨
}
...
# 정보 안에 세부 정보가 또 있을 때 dict 안에 dict를 넣는다.
user = {
    "name": "박서연",
    "address": {                       # 값이 dict
        "city": "부산",
...
# - 안쪽으로 접근할 땐 [키] 또는 [인덱스]를 계속 이어 붙인다.

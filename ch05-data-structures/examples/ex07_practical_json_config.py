...
# 이 예제에서는 [키]와 [인덱스]를 이어 붙여 깊은 값에 접근하는 법을 연습한다.
...
# list 안에 dict가 있으면 [인덱스] 뒤에 [키]를 붙인다.
print("첫 상품명:", response["order"]["items"][0]["name"])


print()
# --- 2. 자주 쓰는 곳은 변수로 짧게 줄이기 ---
order = response["order"]
items = order["items"]


print()
# --- 3. 중첩된 list(dict)를 순회하며 합계 구하기 ---
print("[주문 상품]")
total = 0
for it in items:
...
print()
# --- 5. 깊은 값 수정 ---
order["customer"]["name"] = "박서연(수정)"        # 중첩 dict 값 변경

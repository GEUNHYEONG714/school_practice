# [실무] 실제 Open API 호출 → JSON 파싱 → 화면 출력
#
...
# 값이 (위도, 경도) tuple인 dict. 좌표처럼 한 묶음으로 다루는 값에 잘 어울린다.
CITIES = {
    "서울": (37.5665, 126.9780),
    "부산": (35.1796, 129.0756),
    "제주": (33.4996, 126.5312),
...
# 날씨 코드(WMO) → 한글 설명. API는 숫자만 주므로 사람이 읽을 말로 바꿔 준다.
WEATHER_TEXT = {
...
def fetch_weather(lat, lng):
    ...
    with urllib.request.urlopen(url, timeout=10) as resp:
        raw = resp.read().decode("utf-8")   # 응답 본문은 JSON 형식의 문자열이다.

    # 핵심: JSON 문자열을 파이썬 dict로 변환한다.
    return json.loads(raw)


# 네트워크가 안 될 때를 대비한 예비 데이터 (실제 응답과 같은 구조)
SAMPLE = {
    "current_weather": {
...
for name, coords in CITIES.items():
    lat, lng = coords                       # tuple 언패킹
    try:
        data = fetch_weather(lat, lng)
    except Exception as e:
        # 인터넷이 막혀도 수업이 멈추지 않도록 예비 데이터를 사용한다.
        print(f"[{name}] API 호출 실패({e.__class__.__name__}) → 예시 데이터로 대체")
        data = SAMPLE

    # --- 3. 중첩 dict에서 값 꺼내기 ---
    # data["current_weather"]는 또 다른 dict이므로 그 안에서 [키]로 값을 꺼낸다.
    cw = data["current_weather"]
    temp = cw["temperature"]
    wind = cw["windspeed"]
    code = cw["weathercode"]

    # 숫자 코드를 한글로 바꾼다. 모르는 코드면 기본값을 쓰도록 .get()을 사용한다.
...
print()
# --- 4. 핵심 정리 ---
# - json.loads(문자열)로 dict로 바꾸면 API 응답을 우리가 배운 dict처럼 다룰 수 있다.

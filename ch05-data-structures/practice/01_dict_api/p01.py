...
예제(ex04)에서는 날씨 서버에서 JSON을 받아 파이썬 dict로 다루었습니다.
이번에는 다른 API에서 공휴일 JSON을 받아와 출력해 봅니다.
...
[받아온 JSON의 모양]
  이번 데이터는 "리스트(list) 안에 dict가 여러 개" 들어 있는 형태입니다.
  [
...
[해야 할 일]
  ...
  3. 각 공휴일의 날짜와 한국어 이름을 번호를 붙여 출력한다.
